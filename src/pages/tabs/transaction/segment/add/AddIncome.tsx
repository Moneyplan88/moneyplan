...
const AddIncome: React.FC<{ type: string }> = (props) => {
  ...
  if (userContext.categories.length > 0) {
    kategori = userContext.categories.map((category) => {
      return (
        <IonSelectOption
          key={category.id_transaction_category}
          value={category.id_transaction_category}
          style={{ marginTop: "15px" }}
          className="card-wallet mx-0 "
        >
  ...
  if (userContext.wallet.length > 0) {
    dompet = userContext.wallet.map((wallets) => {
      return (
        <IonSelectOption
          key={wallets.id_user_wallet}
          value={wallets.id_user_wallet}
          style={{ marginTop: "15px" }}
          className="card-wallet mx-0 "
        >
      ...
      <IonContent>
        <IonList>
          ...
          <IonItem class="ion-item">
            ...
            <IonInput
              value={description}
              onIonChange={(e) => setDescrition(e.detail.value!)}
            ></IonInput>
          </IonItem>
          ...
          <IonItem class="ion-item">
            ...
            <IonInput
              ...
              value={amount}
              onIonChange={(e) =>
                setAmount(e.detail.value as unknown as number)
              }
            ></IonInput>
          </IonItem>
